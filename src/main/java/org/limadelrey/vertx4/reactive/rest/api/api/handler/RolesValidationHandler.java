...
@Singleton
public class RolesValidationHandler {
    ...
    private ObjectSchemaBuilder buildBodySchemaBuilder() {
        return objectSchema()
                .requiredProperty("role_name", stringSchema().with(minLength(1)).with(maxLength(255)))
                .requiredProperty("role_prompt", stringSchema().with(minLength(1)).with(maxLength(255)));
