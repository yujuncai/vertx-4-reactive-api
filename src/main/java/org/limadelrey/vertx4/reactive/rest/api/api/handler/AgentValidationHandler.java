...
@Singleton
public class AgentValidationHandler {

    private final Vertx vertx=  Vertx.currentContext().owner();

    public AgentValidationHandler() {
    ...
    private ObjectSchemaBuilder buildBodySchemaBuilder() {
        return objectSchema()
                .requiredProperty("hosts", stringSchema().with(minLength(1)).with(maxLength(255)))
                .requiredProperty("port", intSchema().with(maximum(10000)).nullable())
                .requiredProperty("uri", stringSchema().with(minLength(1)).with(maxLength(255)))
                .requiredProperty("type", stringSchema().with(minLength(1)).with(maxLength(255)))
                .requiredProperty("action", stringSchema().with(minLength(1)).with(maxLength(255)))
                .requiredProperty("apikey", stringSchema().with(minLength(1)).with(maxLength(255)));
    }
