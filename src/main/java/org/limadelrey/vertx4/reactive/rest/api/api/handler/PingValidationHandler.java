...
@Singleton
public class PingValidationHandler {

    private final Vertx vertx=  Vertx.currentContext().owner();

    public PingValidationHandler() {
    ...
    private ObjectSchemaBuilder buildBodySchemaBuilder() {
        return objectSchema()
                ...
                .requiredProperty("role_id", stringSchema().with(minLength(1)).with(maxLength(255)).nullable())
                .requiredProperty("source_id", stringSchema().with(minLength(1)).with(maxLength(255)).nullable())
                .requiredProperty("desc_info", stringSchema().with(minLength(1)).with(maxLength(255)).nullable())
                .requiredProperty("target_id", stringSchema().with(minLength(1)).with(maxLength(255)).nullable());
